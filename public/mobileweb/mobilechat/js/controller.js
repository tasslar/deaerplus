...
webChat.directive('input', function($timeout) {
  return {
    restrict: 'E',
    scope: {
      'returnClose': '=',
    ...
    link: function(scope, element, attr) {
      element.bind('focus', function(e) {
        if (scope.onFocus) {
          $timeout(function() {
            scope.onFocus();
            scope.setFn({theDirFn: scope.onFocus});
          });
        }
      });
...
webChat.controller('buyCtrl', function($scope, $timeout, $ionicScrollDelegate, $http, $stateParams, $location, $anchorScroll, $timeout, $q, apiurl, $ionicHistory) {
  ...
  conversationId = $stateParams.cid;
          ...
          if(oldValue.length==0)
          {
            ...
            angular.forEach(newValue,function(value,key){
              ...
              $scope.items.unshift({'message':value.message,'user_id':value.user_id,'date':value.time});
                ...
                /*scroll start*/
                $location.hash('bottom');
                var handle = $ionicScrollDelegate.$getByHandle('myContent');
                handle.anchorScroll();
...
webChat.controller('sellCtrl', function($scope, $timeout, $ionicScrollDelegate, $http, $stateParams, $location, $anchorScroll, $timeout, $q, apiurl, $ionicHistory) {
