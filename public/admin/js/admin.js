...
function themetype_embedcode(id) {
	if(id=="embedded"){
		...
		window.open('?module=dashboard&action=loadthemetype&type=theme&name=docked&process=true&ts='+ts,'external','width=400,height=200,resizable=1,scrollbars=1');
	}
}
...
function embed_link(url,width,height) {
	...
	if(module[0]=='chatrooms'){
		if(mod[mod.length-1].indexOf('id=') > -1) {
			...
			embedlink.document.write('<textarea readonly style="width:500px;height:140px"><div id="cometchat_embed_chatrooms_container" style="display:inline-block; border:1px solid #CCCCCC;"></div>\n'+synergyembedscript+'</textarea>');
		} else {
